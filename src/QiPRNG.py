...
pseudorandom number generator. All are mathematically equivalent in exact
...
def QiPRNG_tridiag(v0, alpha, beta, M, verbosity=0):
    ...
    # the Hamiltonian
    H = sp.sparse.diags(
        [np.conj(beta), alpha, beta], [-1, 0, 1], dtype=np.complex128
    ).tocsr()
    print(alpha)
    print(M)
    ...
    A = sp.sparse.diags(
        [abs_beta, abs_alpha, abs_beta], [-1, 0, 1], dtype=np.complex128
    )
    ...
    if verbosity >= 1:
        # measuring how close to unitarity we are
        dev = np.max(
            np.abs(W.dot(W.transpose().conjugate(True)).todense() - np.eye(W.shape[0]))
        )
        print("Finished constructing walk operator")
        print("Deviation from unitarity: ", dev)
